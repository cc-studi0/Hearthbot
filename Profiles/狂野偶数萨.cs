...
#region 直伤卡牌 标准模式
        ...
        private static readonly Dictionary<Card.Cards, int> _MinionDamagesTable = new Dictionary<Card.Cards, int>
        {
            ...
            {Card.Cards.RLK_222t2, 14},//火焰使者阿斯塔洛 Astalor, the Flamebringer  RLK_222t2
            {Card.Cards.RLK_224, 2},//监督者弗里吉达拉 Overseer Frigidara  RLK_224
             {Card.Cards.RLK_063, 5},//冰霜巨龙之怒 Frostwyrm's Fury  RLK_063 
            {Card.Cards.RLK_015, 3},//凛风冲击 Howling Blast  RLK_015 
            {Card.Cards.RLK_516, 2},//碎骨手斧 Bone Breaker  RLK_516
            {Card.Cards.WW_336, 3},//WW_336	棱彩光束
        #endregion
				};
#region 攻击模式和自定义 
      ...
      public ProfileParameters GetParameters(Board board)
      {
            _log = "";
         //Bot Logs title
            string Divider1 = new string('=', 27); //string Divider1 is 27 = characters
            string Divider2 = new string('=', 42); //string Divider2 is 33 = characters
            ...
            try
            {
                ...
                AddLog("================ 狂野偶数萨 决策日志 v" + ProfileVersion + " ================");
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            int HandCount = board.Hand.Count;
              // 图腾数量
            int numberTotems = board.MinionFriend.Count(card => card.IsRace(Card.CRace.TOTEM))+board.MinionFriend.Count(card => card.IsRace(Card.CRace.ALL));
			AddLog("图腾数量"+numberTotems);
            int heroHealthLoss = 30-board.HeroFriend.CurrentHealth;
 #endregion
 #region 不送的怪
          // p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.TSC_074, new Modifier(350)); //克托里·光刃 Kotori Lightblade  TSC_074 
#endregion
#region 送的怪
        
          if(board.HasCardOnBoard(Card.Cards.TSC_962)){//修饰老巨鳍 Gigafin  TSC_962 
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.TSC_962t, new Modifier(-100)); //修饰老巨鳍之口 Gigafin's Maw  TSC_962t 
          }
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.REV_373t, new Modifier(-100)); //具象暗影 Shadow Manifestation  REV_373t 
#endregion
#region 技能
  // 如果场上有伴唱机,提高技能优先级
        	...
        	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_528, new Modifier(999));  
        	p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.HERO_02bp, new Modifier(-55)); 
            AddLog("如果场上有伴唱机,提高技能优先级");
            }   
					
          if(board.MaxMana>=7&&friendCount<2&&board.HeroEnemy.Template.Id==Card.Cards.WW_0700){//修饰老巨鳍 Gigafin  TSC_962 
        	p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.HERO_02bp, new Modifier(130)); 
            AddLog("降低技能优先级");
          }
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.HERO_02bp, new Modifier(3999)); 
#endregion

#region DEEP_008	针岩图腾	
          if(board.HasCardInHand(Card.Cards.DEEP_008)
        &&friendCount<=2
        ){
        	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DEEP_008, new Modifier(150)); 
					AddLog("针岩图腾150");
          }else{
        	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DEEP_008, new Modifier(-150)); 
            AddLog("针岩图腾-150");
					}
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.DEEP_008, new Modifier(350)); 
           if(board.HasCardInHand(Card.Cards.DEEP_008)
           &&heroHealthLoss>=10
		){
        	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DEEP_008, new Modifier(-30*heroHealthLoss)); 
		AddLog("针岩图腾"+(-30*heroHealthLoss));
          }
#endregion
#region Card.Cards.TOY_528 // TOY_528	伴唱机 TOY_528	伴唱机
        	p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TOY_528, new Modifier(8999));  
            if(board.HasCardInHand(Card.Cards.TOY_528)
            &&board.MaxMana>=3
            ){
        	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_528, new Modifier(-120));  
			AddLog("伴唱机 -120");
            }
            if(board.HasCardInHand(Card.Cards.TOY_528)
            &&board.MaxMana<3
            ...
            // 不送
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.TOY_528, new Modifier(350)); 
          
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WW_027, new Modifier(999,Card.Cards.TOY_528));//WW_027	可靠陪伴
            p.ForcedResimulationCardList.Add(Card.Cards.TOY_528);

#endregion
#region TTN_800	雷霆之神高戈奈斯
            if(board.HasCardInHand(Card.Cards.TTN_800)
            ){
        	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TTN_800, new Modifier(150));  
			AddLog("雷霆之神高戈奈斯 150");
            }
#endregion
#region TTN_710	远古图腾
          if(board.HasCardInHand(Card.Cards.TTN_710)){
        	p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TTN_710, new Modifier(500)); 
			AddLog("远古图腾优先级500");
          }
#endregion
#region WC_020	永恒之火
			p.PlayOrderModifiers.AddOrUpdate(Card.Cards.WC_020, new Modifier(-99)); 
          if(board.HasCardInHand(Card.Cards.WC_020)){
        	p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WC_020, new Modifier(300)); 
			AddLog("永恒之火300");
          }
#endregion
 #region ULD_177	八爪巨怪
 ...
 #endregion
 #region WON_091	图腾团聚
        var tuanju = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.WON_091);
// 图腾之力 Totemic Might  EX1_244 
        var zhili = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.EX1_244);
				
						if(tuanju!=null
						&&friendCount<=3
						){
              p.ComboModifier = new ComboSet(tuanju.Id);
						AddLog("图腾团聚出");
						}else if(tuanju!=null
						&&zhili!=null
						&&friendCount<=3){
							p.ComboModifier = new ComboSet(tuanju.Id,zhili.Id);
						AddLog("图腾团聚+图腾之力出");
						}else{
						p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WON_091, new Modifier(200)); 
						}
 #endregion
 #region JAM_013	先祖知识
						...
						p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_AT_053, new Modifier(-20)); 
						AddLog("先祖知识-20");
						}else{
 ...
 #region JAM_013	即兴演奏
						...
						p.PlayOrderModifiers.AddOrUpdate(Card.Cards.JAM_013, new Modifier(-999)); 
						AddLog("即兴演奏优先级999");
						}	
							if(board.HasCardInHand(Card.Cards.JAM_013)
						){
...
p.ForcedResimulationCardList.Add(Card.Cards.JAM_013);
 #endregion
 #region CORE_EX1_565	火舌图腾
						if(board.HasCardInHand(Card.Cards.CORE_EX1_565)
						&&friendCount>0
						){
						p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CORE_EX1_565, new Modifier(-10)); 
						AddLog("火舌图腾 -10");
						}	
						p.PlayOrderModifiers.AddOrUpdate(Card.Cards.CORE_EX1_565, new Modifier(999)); 
 #endregion
 #region GVG_038	连环爆裂
	// 敌方英雄血量小于等于12,提高连环爆裂打脸的可能
				if(board.HasCardInHand(Card.Cards.GVG_038)
				&&board.HeroEnemy.CurrentHealth<=12
				){
						p.CastSpellsModifiers.AddOrUpdate(SmartBot.Plugins.API.Card.Cards.GVG_038, -999, board.HeroEnemy.Id);
						AddLog("敌方英雄血量小于等于12,提高连环爆裂打脸的可能");
				}
				// 第三回合降低连环闪电优先级,手里有图腾师或者分裂战斧
				if(board.HasCardInHand(Card.Cards.GVG_038)
				&&board.MaxMana==3
				&&(board.HasCardInHand(Card.Cards.ULD_413)||board.HasCardInHand(Card.Cards.AT_052))
				){
						p.CastSpellsModifiers.AddOrUpdate(SmartBot.Plugins.API.Card.Cards.GVG_038, 150);
						AddLog("第三回合降低连环闪电优先级,手里有图腾师或者分裂战斧");
				}
            p.ForcedResimulationCardList.Add(Card.Cards.GVG_038);

 #endregion
 #region 图腾之力 Totemic Might  EX1_244 
				 if (board.HasCardInHand(Card.Cards.EX1_244)
            &&(numberTotems >= 3||(numberTotems >= 2&&board.MinionEnemy.Count>0))
        ) {
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_244, new Modifier(-20*numberTotems)); //图腾之力 Totemic Might   EX1_244
        }else{
						p.CastSpellsModifiers.AddOrUpdate(Card.Cards.EX1_244, new Modifier(350)); //图腾之力 Totemic Might   EX1_244
				}
       p.PlayOrderModifiers.AddOrUpdate(Card.Cards.EX1_244, new Modifier(-50));
       
#endregion
#region  图腾潮涌 Totemic Surge  ULD_171 
        if (board.HasCardInHand(Card.Cards.ULD_171)
            &&numberTotems < 3
        ) {
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ULD_171, new Modifier(350));
        }else{
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ULD_171, new Modifier(150));
				}
        //如果对方是瞎子，则提高图腾潮涌的优先级
        if (board.EnemyClass == Card.CClass.DEMONHUNTER
            && numberTotems < 2
            &&board.HasCardInHand(Card.Cards.ULD_171)
        ) {
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ULD_171, new Modifier(350));
        }
        else{
        ...
        // p.PlayOrderModifiers.AddOrUpdate(Card.Cards.ULD_171, new Modifier(-999));
#endregion


 #region 笼斗管理员 Cagematch Custodian  DMF_704 
            if(board.HasCardInHand(Card.Cards.DMF_704)
            &&!board.HasCardInHand(Card.Cards.REV_917)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_704, new Modifier(-99)); 
            AddLog("笼斗管理员 -99");
            }
#endregion
 #region 石雕凿刀 REV_917 
            if(board.HasCardInHand(Card.Cards.REV_917)
            &&board.WeaponFriend == null
						&&(board.MaxMana==1&&board.MinionEnemy.Count>0)||(board.MaxMana>1)
            ){
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.REV_917, new Modifier(-99));
                AddLog("石雕凿刀 -99");
            }else{
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.REV_917, new Modifier(999));
						}
						if( board.WeaponFriend != null 
						&& board.WeaponFriend.Template.Id == Card.Cards.REV_917
						&&friendCount>=6){
                p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.REV_917, new Modifier(999));
                AddLog("场上随从大于5,降低石雕凿刀攻击优先级");
					}
p.ForcedResimulationCardList.Add(Card.Cards.REV_917);

#endregion
 #region 深海融合怪 TSC_069
					if(board.HasCardInHand(Card.Cards.TSC_069)
						&&board.MinionFriend.Count ==0
						){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_069, new Modifier(150)); 
            AddLog("深海融合怪 150");
            }
						// 遍历场上随从,如果类型是图腾,提高深海融合怪使用优先级,否则则不用
						foreach (var minion in board.MinionFriend)
						{
								if (!minion.IsRace(Card.CRace.TOTEM)
								&&board.HasCardInHand(Card.Cards.TSC_069)
								)
								{
										p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_069, new Modifier(150,minion.Template.Id));
										// AddLog("深海融合怪 150");
								}
						}
#endregion
#region 驻锚图腾 TSC_922 
						 if (board.HasCardInHand(Card.Cards.TSC_922)
						&&((board.WeaponFriend != null 
						&& board.WeaponFriend.Template.Id == Card.Cards.REV_917
						)||board.MaxMana>2)
            ) {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(-250)); 
            		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_069, new Modifier(200)); 
								AddLog("驻锚图腾-250");
            }else{
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(200)); 
            }
								p.PlayOrderModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(999));
          p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.TSC_922, new Modifier(350));  
 ...
 #region AV_259	冰霜撕咬
           // 遍历场上随从,如果类型是图腾,提高可靠陪伴
						...
						{
							// 血量为3,提高优先级
								if (minion.CurrentHealth ==3
								&&board.HasCardInHand(Card.Cards.AV_259)
								)
								{
										p.CastSpellsModifiers.AddOrUpdate(Card.Cards.AV_259, new Modifier(-99,minion.Template.Id));
										// AddLog("冰霜撕咬 150");
								}
						}
#endregion
 #region WW_027	可靠陪伴
           // 遍历场上随从,如果类型是图腾,提高可靠陪伴
						...
						{
								if (!minion.IsRace(Card.CRace.TOTEM)
								&&board.HasCardInHand(Card.Cards.WW_027)
								)
								{
								...
								// 可攻击
								// if (minion.CanAttack&&minion.IsRace(Card.CRace.TOTEM)){
								// 		p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WW_027, new Modifier(-99,minion.Template.Id));
								// }
						}
						p.PlayOrderModifiers.AddOrUpdate(Card.Cards.WW_027, new Modifier(999));
#endregion
 #region SCH_713	异教低阶牧师
...
#endregion 
 #region GIL_692	吉恩·格雷迈恩
           if(board.HasCardInHand(Card.Cards.GIL_692)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.GIL_692, new Modifier(150)); 
            AddLog("吉恩·格雷迈恩 150");
            }
#endregion 
 #region 锻石师 The Stonewright  REV_921  
           if(board.HasCardInHand(Card.Cards.REV_921)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_921, new Modifier(-99-(numberTotems*60))); 
            AddLog("锻石师"+(-99-(numberTotems*60)));
            }
#endregion 
 #region 图腾魔像 AT_052
            if(board.HasCardInHand(Card.Cards.AT_052)
            &&board.MaxMana !=3
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.AT_052, new Modifier(-99)); 
            AddLog("图腾魔像 -99");
            }
#endregion
 #region   GIL_530, // 阴燃电鳗 
            if(board.HasCardInHand(Card.Cards.GIL_530)
						&&board.MinionEnemy.Count ==0
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.GIL_530, new Modifier(200)); 
            AddLog("阴燃电鳗 200");
            }
#endregion
 #region 怪盗图腾 EVIL Totem  ULD_276 
            if(board.HasCardInHand(Card.Cards.ULD_276)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.ULD_276, new Modifier(-99)); 
            AddLog("怪盗图腾 -99");
            }
#endregion
#region 分裂战斧 Splitting Axe   ULD_413
            // 费用等于4，手上有分裂斧且场上无图腾，降低分裂战斧优先级，优先摇图腾
                if (board.HasCardInHand(Card.Cards.ULD_413)
                &&(numberTotems <=1||board.MinionFriend.Count >=6 )
                )
                {
                p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.ULD_413, new Modifier(999)); //分裂战斧 Splitting Axe   ULD_413
                p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.CS2_049_H1, new Modifier(-20));//图腾召唤 Totemic Call   CS2_049_H1
                }
                p.ForcedResimulationCardList.Add(Card.Cards.ULD_413);
#endregion
#region 深渊魔物 CORE_OG_028
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.CORE_OG_028, new Modifier(-50));
            if(
						board.Hand.Exists(x=>x.CurrentCost<=0 && x.Template.Id==Card.Cards.CORE_OG_028)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CORE_OG_028, new Modifier(-99));
						AddLog("深渊魔物-99");
            }
#endregion
#region 图腾巨像 Gigantotem  REV_838 
						p.PlayOrderModifiers.AddOrUpdate(Card.Cards.REV_838, new Modifier(60));
						if(
						board.Hand.Exists(x=>x.CurrentCost<=0 && x.Template.Id==Card.Cards.REV_838)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.REV_838, new Modifier(-99));
						AddLog("图腾巨像-99");
            }
#endregion
// 公共区
#region GAME_005
        // if(board.MaxMana){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(35));
        // }
#endregion
#region 战场军官  SW_063
...
#endregion
 #region 痛苦诅咒 Curse of Agony  ONY_034
            if(board.HasCardInHand(Card.Cards.ONY_034)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.ONY_034, new Modifier(-99)); 
            //AddLog("痛苦诅咒 -99");
            }
#endregion
#region 阿斯塔洛·血誓 Astalor Bloodsworn  RLK_222 
    if(board.HasCardInHand(Card.Cards.RLK_222)
    &&board.MaxMana<5){
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.RLK_222, new Modifier(999)); 
    }else{
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.RLK_222, new Modifier(-99)); 
    }
#endregion
#region 护卫阿斯塔洛 Astalor, the Protector  RLK_222t1
    if(board.HasCardInHand(Card.Cards.RLK_222t1)
    &&board.MaxMana<8){
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.RLK_222t1, new Modifier(999)); 
    }else{
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.RLK_222t1, new Modifier(-99)); 
    }
#endregion
#region 火焰使者阿斯塔洛 Astalor, the Flamebringer  RLK_222t2 
    if(board.HasCardInHand(Card.Cards.RLK_222t2)
    &&board.MaxMana<10){
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.RLK_222t2, new Modifier(999)); 
    }
#endregion
...
#endregion
#region 吞噬者穆坦努斯 Mutanus the Devourer  WC_030 
       if(board.HasCardInHand(Card.Cards.WC_030)){
       	p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WC_030, new Modifier(-150)); 
        //AddLog("吞噬者穆坦努斯 -150");
      }
#endregion
#region 洛瑟玛·塞隆 Lor'themar Theron  RLK_593 
      if(board.HasCardInHand(Card.Cards.RLK_593)
      ){
		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.RLK_593, new Modifier(-150));
        //AddLog("洛瑟玛·塞隆 -150");
        }
#endregion
#region 战争之骑士瑞文戴尔 Rivendare, Warrider  NX2_034 
      if(board.HasCardInHand(Card.Cards.NX2_034)
      ){
		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.NX2_034, new Modifier(-250));
        //AddLog("战争之骑士瑞文戴尔 -250");
        }
#endregion
#region 饥荒之骑士布劳缪克丝 Blaumeaux, Faminerider  NX2_034t
      if(board.HasCardInHand(Card.Cards.NX2_034t)
      ){
		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.NX2_034t, new Modifier(-99));
        //AddLog("饥荒之骑士布劳缪克丝 -99");
        }
#endregion
#region 死亡之骑士库尔塔兹 Korth'azz, Deathrider  NX2_034t1 
      if(board.HasCardInHand(Card.Cards.NX2_034t1)
      ){
		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.NX2_034t1, new Modifier(-99));
        //AddLog("死亡之骑士库尔塔兹 -99");
        }
#endregion

#region 征服之骑士瑟里耶克 Zeliek, Conquestrider  NX2_034t2 
      if(board.HasCardInHand(Card.Cards.NX2_034t2)
      ){
		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.NX2_034t2, new Modifier(-99));
        //AddLog("征服之骑士瑟里耶克 -99");
        }
#endregion
