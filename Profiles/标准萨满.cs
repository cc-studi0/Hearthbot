...
#region 攻击模式和自定义 
      ...
      public ProfileParameters GetParameters(Board board)
      {
            ...
            try
            {
                ...
                AddLog($"================ 标准萨满 决策日志 v{ProfileVersion} ================");
...
// 熔爪巨龙 TLC_482 随从 标记态 且友方随从小于等于4
# region 熔爪巨龙 TLC_482
				if(board.Hand.Exists(card => GetTag(card, Card.GAME_TAG.POWERED_UP) == 1 && card.Template.Id == Card.Cards.TLC_482)
				&& board.MinionFriend.Count <= 4
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TLC_482, new Modifier(-150)); 
					AddLog("熔爪巨龙 -150");
				}else{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TLC_482, new Modifier(350)); 
				}
#endregion

// 火山长尾蜥 TLC_223 标记状态
# region 火山长尾蜥 TLC_223
				if(board.Hand.Exists(card => GetTag(card, Card.GAME_TAG.POWERED_UP) == 1 && card.Template.Id == Card.Cards.TLC_223)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TLC_223, new Modifier(-150)); 
					AddLog("火山长尾蜥 -150");
				}else{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TLC_223, new Modifier(350)); 
				}
#endregion

// 极紫外破坏者 GDB_901 
# region 极紫外破坏者 GDB_901
				if(board.HasCardInHand(Card.Cards.GDB_901)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.GDB_901, new Modifier(-150)); 
					AddLog("极紫外破坏者 -150");
				}
#endregion
...
// 烬鳍鱼人 TLC_225 随从
# region 烬鳍鱼人 TLC_225
				if(board.HasCardInHand(Card.Cards.TLC_225)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TLC_225, new Modifier(-150)); 
					AddLog("烬鳍鱼人 -150");
				}
#endregion

// 凶恶的雨云 CORE_BOT_533 随从
# region 凶恶的雨云 CORE_BOT_533
				if(board.HasCardInHand(Card.Cards.CORE_BOT_533)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CORE_BOT_533, new Modifier(-150)); 
					AddLog("凶恶的雨云 -150");
				}
#endregion
...
// 炽烈烬火 TLC_249 随从
# region 炽烈烬火 TLC_249
				if(board.HasCardInHand(Card.Cards.TLC_249)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TLC_249, new Modifier(-150)); 
					AddLog("炽烈烬火 -150");
				}
#endregion
...
// 哀嚎蒸汽 WC_042 随从
# region 哀嚎蒸汽 WC_042
				if(board.HasCardInHand(Card.Cards.WC_042)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WC_042, new Modifier(-150)); 
					p.PlayOrderModifiers.AddOrUpdate(Card.Cards.WC_042, new Modifier(999)); 
					AddLog("哀嚎蒸汽 -150");
				}else{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WC_042, new Modifier(350)); 
				}
#endregion

// 冰霜摆件 VAC_305 法术 友方随从数小于等于5
# region 冰霜摆件 VAC_305
				if(board.HasCardInHand(Card.Cards.VAC_305)
				&& board.MinionFriend.Count <= 5
				)
				{
					p.CastSpellsModifiers.AddOrUpdate(Card.Cards.VAC_305, new Modifier(-350)); 
					AddLog("冰霜摆件 -350");
				}else{
					p.CastSpellsModifiers.AddOrUpdate(Card.Cards.VAC_305, new Modifier(350)); 
				}
...
// 高山之王穆拉丁 TIME_209 随从
# region 高山之王穆拉丁 TIME_209
				if(board.HasCardInHand(Card.Cards.TIME_209)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TIME_209, new Modifier(-350)); 
					AddLog("高山之王穆拉丁 -350");
				}else{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TIME_209, new Modifier(350)); 
				}
...
// 武器 高山之王的战锤 TIME_209t
# region 高山之王的战锤 TIME_209t
				if(board.HasCardInHand(Card.Cards.TIME_209t)
				)
				{
					p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.TIME_209t, new Modifier(-350)); 
					AddLog("高山之王的战锤 -350");
				}else{
					p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.TIME_209t, new Modifier(350)); 
				}
...
// 随从 童话泥浆怪 TOY_504t
# region 童话泥浆怪 TOY_504t
				if(board.HasCardInHand(Card.Cards.TOY_504t)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_504t, new Modifier(-350)); 
					AddLog("童话泥浆怪 -350");
				}else{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_504t, new Modifier(350)); 
				}
...
//地标 鹦鹉乐园 VAC_409
# region 鹦鹉乐园 VAC_409
					p.LocationsModifiers.AddOrUpdate(Card.Cards.VAC_409, new Modifier(-150)); 
					p.PlayOrderModifiers.AddOrUpdate(Card.Cards.VAC_409, new Modifier(999)); 
#endregion

//法术 观赏鸟类 VAC_408
# region 观赏鸟类 VAC_408
var birdCard = board.Hand.FirstOrDefault(card => card.Template.Id == Card.Cards.VAC_408);
				if(birdCard!=null
				)
				{
					// p.CastSpellsModifiers.AddOrUpdate(Card.Cards.VAC_408, new Modifier(-999)); 
					p.ComboModifier = new ComboSet(birdCard.Id);
					AddLog("观赏鸟类出");
				}
#endregion

// 始源监督者 TIME_213
# region 始源监督者 TIME_213
				if(board.Hand.Exists(card => GetTag(card, Card.GAME_TAG.POWERED_UP) == 1 && card.Template.Id == Card.Cards.TIME_213)
				)
				{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TIME_213, new Modifier(-150)); 
					AddLog("始源监督者 -150");
				}else{
					p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TIME_213, new Modifier(350)); 
				}
#endregion
/* 连招，沙德木刻和其他战吼 当有手上的费用足够时 下沙德木刻和战吼的combeo
TOY_501 大沙德木刻 6费
TOY_501t 小沙德木刻 1费
MIS_307 小水宝宝鱼人 1费 使用条件 场上随从小于等于3
MIS_307t1 大水宝宝鱼人 8费 使用条件 场上随从小于等于3
TIME_213 始源监督者 2费 使用条件 始源监督者为标记状态
VAC_521 笨拙的搬运工 3费 使用条件 笨拙的搬运工为标记状态 场上随从小于等于3
WORK_013 湍流元素特布勒斯 3费
TOY_504 神秘女巫哈加莎 4费 使用条件 手上牌小于等于5
TLC_100 导航员伊莉斯 4费 手牌数小于等于7
童话泥浆怪 TOY_504t
伊辛迪奥斯 VAC_321 6费
当场上有大沙德木刻 且当前费用不为满 比如六费只剩5费 不用小沙德木刻 留着下回合用
当满足条件且费用足够时 触发连招
如果满足条件但费用不够 降低组件使用优先级
*/ 

#region 沙德木刻连招逻辑
// 获取手牌中的沙德木刻（优先使用1费的微缩版本）
var shudderblock = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.TOY_501); // 6费
var miniShudderblock = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.TOY_501t); // 1费

Card currentShudderblock = miniShudderblock ?? shudderblock;
bool comboFound = false;

if (currentShudderblock != null)
{
    // 定义连招处理函数
    void HandleShudderblockCombo(Card.Cards targetId, Func<Card, bool> extraCondition = null)
    {
        var targetCard = board.Hand.FirstOrDefault(x => x.Template.Id == targetId);
        if (targetCard != null)
        {
            // 检查是否有额外条件
            bool conditionMet = extraCondition == null || extraCondition(targetCard);
            
            if (conditionMet)
            {
                 // 检查法力值是否足够同时打出两张牌
                if (board.ManaAvailable >= (currentShudderblock.CurrentCost + targetCard.CurrentCost))
                {
                    // 设置组合：先下沙德木刻，再下战吼怪
                    p.ComboModifier = new ComboSet(currentShudderblock.Id, targetCard.Id);
                    
                    // 提高两张卡的优先级，确保打出
                    p.CastMinionsModifiers.AddOrUpdate(currentShudderblock.Template.Id, new Modifier(-999));
                    p.CastMinionsModifiers.AddOrUpdate(targetCard.Template.Id, new Modifier(-900));
                    
                    AddLog($"触发沙德木刻连招: {currentShudderblock.Template.NameCN} + {targetCard.Template.NameCN}");
                    comboFound = true;
                }
                else
                {
                    // 如果满足条件但费用不够 降低组件使用优先级
                    // 如果已经找到连招，就不降低沙德木刻的优先级了
                    if (!comboFound)
                    {
                            p.CastMinionsModifiers.AddOrUpdate(currentShudderblock.Template.Id, new Modifier(350));
                    }
                    p.CastMinionsModifiers.AddOrUpdate(targetCard.Template.Id, new Modifier(350));
                    AddLog($"费用不足以触发沙德木刻连招，保留组件: {currentShudderblock.Template.NameCN} + {targetCard.Template.NameCN}");
                }
            }
        }
    }
    ...
    // MIS_307 小水宝宝鱼人 1费 (场上随从 <= 3)
    HandleShudderblockCombo(Card.Cards.MIS_307, c => board.MinionFriend.Count <= 3);

    // MIS_307t1 大水宝宝鱼人 8费 (场上随从 <= 3)
    HandleShudderblockCombo(Card.Cards.MIS_307t1, c => board.MinionFriend.Count <= 3);

    // TIME_213 始源监督者 2费 (标记状态)
    HandleShudderblockCombo(Card.Cards.TIME_213, c => GetTag(c, Card.GAME_TAG.POWERED_UP) == 1);

    // VAC_521 笨拙的搬运工 3费 (标记状态 && 场上随从 <= 3)
    HandleShudderblockCombo(Card.Cards.VAC_521, c => GetTag(c, Card.GAME_TAG.POWERED_UP) == 1 && board.MinionFriend.Count <= 3);

    // WORK_013 湍流元素特布勒斯 3费 (无特殊条件)
    HandleShudderblockCombo(Card.Cards.WORK_013);

    // TOY_504 神秘女巫哈加莎 4费 (手牌数 <= 5)
    HandleShudderblockCombo(Card.Cards.TOY_504, c => board.Hand.Count <= 5);

    // TLC_100 导航员伊莉斯 4费 (手牌数 <= 7)
    HandleShudderblockCombo(Card.Cards.TLC_100, c => board.Hand.Count <= 7);
		// TOY_504t 童话泥浆怪 5费 (无特殊条件)
		HandleShudderblockCombo(Card.Cards.TOY_504t);
		// 伊辛迪奥斯 VAC_321 6费 (无特殊条件)
		HandleShudderblockCombo(Card.Cards.VAC_321);
}

// 其他情况下 降低小沙德木刻的使用优先级
if (!comboFound && miniShudderblock != null)
{
     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_501t, new Modifier(350));
}

// 当场上有大沙德木刻 且当前费用不为满 比如六费只剩5费 不用小沙德木刻 留着下回合用
if (board.HasCardOnBoard(Card.Cards.TOY_501) && board.ManaAvailable < board.MaxMana)
{
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_501t, new Modifier(999));
    AddLog("场上有大沙德木刻且费用不满，保留小沙德木刻");
}
#endregion

// TOY_877 星空祈愿
# region 星空祈愿 TOY_877
				if(board.HasCardInHand(Card.Cards.TOY_877)
				)
				{
					p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TOY_877, new Modifier(-350)); 
					AddLog("星空祈愿 -350");
				}else{
					p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TOY_877, new Modifier(350)); 
				}
...
// TLC_222 火鹰飞翔
# region 火鹰飞翔 TLC_222
				if(board.HasCardInHand(Card.Cards.TLC_222)
				// 手牌数小于等于9
				&& board.Hand.Count <= 9
				)
				{
					p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TLC_222, new Modifier(-150)); 
					AddLog("火鹰飞翔 -150");
				}else{
					p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TLC_222, new Modifier(350)); 
				}
#endregion
...
#region 神秘女巫哈加莎 TOY_504
        if(board.HasCardInHand(Card.Cards.TOY_504)
				// 手牌数小于等于8
				&& board.Hand.Count <= 8
        )
        {
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_504, new Modifier(-350)); 
          AddLog("神秘女巫哈加莎 -350");
        }
#endregion
...
#region 湍流元素特布勒斯 WORK_013
        ...
        {
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WORK_013, new Modifier(-350)); 
          AddLog("湍流元素特布勒斯 -350");
        }
#endregion
...
#region  立体书 TOY_508
					// 5费的时候,如果手上有礼物,随从数小于等于5,提高立体书的使用优先级
					if(board.HasCardInHand(Card.Cards.TOY_508)
					&&board.MaxMana>=5
					&&board.Hand.Exists(card => card.Template.Id == Card.Cards.GIFT_06)
					&&friendCount<=5
					// 当前攻击力+(当前随从数量+2)*3大于等于敌方英雄血量,敌方无嘲讽,提高使用优先级
					&&myAttack+(friendCount+2)*3>=board.HeroEnemy.CurrentHealth+board.HeroEnemy.CurrentArmor
					&&!board.MinionEnemy.Exists(x=>x.IsTaunt)
					){
						p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TOY_508, new Modifier(-150));
						AddLog("立体书 -150");
					}else if(board.HasCardInHand(Card.Cards.TOY_508)
				// 友方随从小于等于6
				&&friendCount<=3
				){
				p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TOY_508, new Modifier(-5)); 
				AddLog("立体书 -5");
        }
#endregion


#region 水宝宝鱼人 MIS_307
	// 如果随从数大于6,降低使用优先级
	if(board.HasCardInHand(Card.Cards.MIS_307)
	&&board.MinionFriend.Count>=7
	){
		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.MIS_307, new Modifier(350));
		AddLog("水宝宝鱼人350");
	}
#endregion

#region 水宝宝鱼人 MIS_307t1
	// 如果随从数大于6,降低使用优先级
	if(board.HasCardInHand(Card.Cards.MIS_307t1)
	&&board.MinionFriend.Count>=7
	){
		p.CastMinionsModifiers.AddOrUpdate(Card.Cards.MIS_307t1, new Modifier(350));
		AddLog("水宝宝鱼人350");
	}
#endregion
...
#region Card.Cards.HERO_02bp 英雄技能
        ...
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.HERO_02bp, new Modifier(999));
				// 
...
var excludedCards = new HashSet<string>
{
    ...
    Card.Cards.TOY_501.ToString(),
    Card.Cards.TOY_501t.ToString(),
		// 蛮鱼挑战者 TLC_251
