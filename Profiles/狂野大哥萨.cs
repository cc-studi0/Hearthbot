...
#region 攻击模式和自定义 
      ...
      public ProfileParameters GetParameters(Board board)
      {
            ...
            try
            {
                ...
                AddLog($"================ 狂野大哥萨 决策日志 v{ProfileVersion} ================");
    ...
    switch (enemyClass)
    {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            {
                Card.Cards.TSC_639, // 暴食巨鳗格拉格
                Card.Cards.WW_440,  // 奔雷骏马
                Card.Cards.WW_382,  // 步移山丘
                Card.Cards.TID_712, // 猎潮者耐普图隆
...
#region 拍卖行木槌 SW_025
    //  如果手上没有战吼随从 泥沼变形怪 DAL_052 ,降低武器攻击值
...
#region 童话林地 TOY_507
    //  如果后手，可以在 2 费跳币上地标（童话林地）
    if(board.HasCardInHand(Card.Cards.TOY_507)
		// 且手上的泥沼变形怪 DAL_052数量为0
...
#region 即兴演奏 JAM_013
    ...
    if(board.HasCardInHand(Card.Cards.JAM_013)
		...
		// 如果场上同时有猎潮者耐普图隆 TID_712 和 (TID_712t 或 TID_712t2),降低即兴演奏对猎潮者耐普图隆 TID_712使用优先级
...
#region 鱼群聚集 TSC_631
 ...
 #region 降低过载牌逻辑
    // 如果下回合可以用泥沼变形怪 DAL_052,降低过载牌优先级
		if(board.Hand.Exists(x=>x.CurrentCost<=board.MaxMana+1 && x.Template.Id==Card.Cards.DAL_052)
		){
				p.CastSpellsModifiers.AddOrUpdate(Card.Cards.JAM_013, new Modifier(350)); //即兴演奏 JAM_013
				p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_AT_053, new Modifier(350)); //CORE_AT_053 先祖知识
				AddLog("降低过载牌逻辑1");
		}
    // 如果下回合可以用先祖召唤 GVG_029 且有大哥,降低过载牌优先级
    ...
    // 如果下回合可以用我找到了 BOT_099 且有大哥,降低过载牌优先级
...
#region 硬币 GAME_005
    // 如果一费手上有硬币,有童话林地 TOY_507,降低硬币优先级
    if(board.MaxMana==1
