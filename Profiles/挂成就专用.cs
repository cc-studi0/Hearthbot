...
#region 攻击模式和自定义 
      ...
      public ProfileParameters GetParameters(Board board)
      {
            ...
            try
            {
                ...
                AddLog($"================ 挂成就专用 决策日志 v{ProfileVersion} ================");
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            int shadowCount = board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.VAC_931);
              // 本轮使用过的元素 board.CardsPlayedThisTurn
            // int usedElement=board.CardsPlayedThisTurn.Count(card => card.IsRace(Card.CRace.PIRATE));
...
#region 恋旧风潮 MIS_701
        if(board.HasCardInHand(Card.Cards.MIS_701)
        )
        {
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.MIS_701, new Modifier(-150)); 
          p.PlayOrderModifiers.AddOrUpdate(Card.Cards.MIS_701, new Modifier(999)); 
          AddLog("恋旧风潮 -150");
        }
#endregion

#region WORK_030 冰冠堡垒宣传单
        var iceCrownFortressFlyer = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.WORK_030);
     if (board.MinionEnemy.Count >=2
     &&iceCrownFortressFlyer!=null
     && board.HasCardInHand(Card.Cards.WORK_030))
    {
        // 遍历敌方随从,对其使用
        foreach (var item in board.MinionEnemy)
        {
            p.ComboModifier = new ComboSet(iceCrownFortressFlyer.Id);
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WORK_030, new Modifier(-350, item.Template.Id));
            AddLog("冰冠堡垒宣传单 -350"+item.Template.Id);
        }
    }
#endregion
...
#region WW_359 桶沿警长 
// 随从大于等于6不用
if(board.HasCardInHand(Card.Cards.WW_359)
&&board.MinionFriend.Count<6
// 当前血量大于等于20
&&(board.HeroFriend.CurrentHealth<20||enemyAttack>8)
){
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_359, new Modifier(-150));
    AddLog("桶沿警长 -150");
}else{
     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_359, new Modifier(350));
}
// 如果已叫杀,且对面有嘲讽,我方随从数小于等于5,提高优先级
if(board.HasCardInHand(Card.Cards.WW_359)
&&board.MinionEnemy.Exists(x=>x.IsTaunt)
&&board.MinionFriend.Count<=5
&&myAttack>board.HeroEnemy.CurrentHealth+board.HeroEnemy.CurrentArmor
){
    p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_359, new Modifier(-150));
    AddLog("桶沿警长 -150");
}
...
#region WW_359t 荒芜之地监狱 Badlands Jail
if(board.HasCardOnBoard(Card.Cards.WW_359t)){
    // 遍历随从,降低对己方随从使用优先值
    foreach (var item in board.MinionFriend)
    {
        p.LocationsModifiers.AddOrUpdate(Card.Cards.WW_359t, new Modifier(999, item.Template.Id)); 
    }
    // 遍历敌方随从,提高对敌方随从使用优先值
    foreach (var item in board.MinionEnemy)
    {
        p.LocationsModifiers.AddOrUpdate(Card.Cards.WW_359t, new Modifier(-20*item.CurrentAtk, item.Template.Id)); 
        // 如果已叫杀,提高对嘲讽的优先值
        if(myAttack>board.HeroEnemy.CurrentHealth+board.HeroEnemy.CurrentArmor
        &&item.IsTaunt
        ){
            p.LocationsModifiers.AddOrUpdate(Card.Cards.WW_359t, new Modifier(-150, item.Template.Id));
            AddLog("如果已叫杀,提高对嘲讽的优先值"+item.Template.Id);
        }
    }
}
#endregion

#region id":"WW_382","mechanics":["LIFESTEAL","OVERLOAD","RUSH"],"name":"步移山丘
    // 提高优先级
    if(board.HasCardInHand(Card.Cards.WW_382)
    ){
        p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_382, new Modifier(-150));
        AddLog("步移山丘 -150");
    }
#endregion
...
#region WW_044t	淤泥桶
				if(board.Hand.Exists(x=>x.CurrentCost==3 && x.Template.Id==Card.Cards.WW_044t)
			){
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WW_044t, new Modifier(-25)); 
				 AddLog("淤泥桶 -25");
			}
			if(
				board.Hand.Exists(x=>x.CurrentCost==1 && x.Template.Id==Card.Cards.WW_044t)
			){
         p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WW_044t, new Modifier(-150)); 
				 AddLog("淤泥桶 -150");
			}
#endregion


#region WW_091	腐臭淤泥波普加
			// 手上淤泥桶数量+2-4等于当前费用,提高优先级
			// WW_044t	淤泥桶
			var fuchou = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.WW_091);
			var yunitonga = board.Hand.FirstOrDefault(x => x.Template.Id == Card.Cards.WW_044t);
			if(board.ManaAvailable>=6
				&&fuchou!=null
			){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_091, new Modifier(-150)); 
				 AddLog("腐臭淤泥波普加 -150");
			}else{
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_091, new Modifier(350)); 
            }
            // 差八血斩杀,赌淤泥桶
            if(board.ManaAvailable>=6
				&&fuchou!=null
                &&myAttack+8>=BoardHelper.GetEnemyHealthAndArmor(board)
			){
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_091, new Modifier(-150)); 
				 AddLog("差八血斩杀,赌淤泥桶");
			}
			...
			// WW_091	腐臭淤泥波普加
			if(board.HasCardOnBoard(Card.Cards.WW_091)
            &&!board.Hand.Exists(x=>x.CurrentCost==0 && x.Template.Id==Card.Cards.CORE_EX1_620)
			&&yunitonga!=null
            // 场攻+8斩杀
            &&myAttack+8>=BoardHelper.GetEnemyHealthAndArmor(board)
			){
                 p.ComboModifier = new ComboSet(yunitonga.Id); 
				 AddLog("淤泥桶出");
			}
          p.PlayOrderModifiers.AddOrUpdate(Card.Cards.WW_091, new Modifier(999)); 
#endregion

#region 弃暗投明 VAC_941
        if(board.HasCardInHand(Card.Cards.VAC_941)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.VAC_941, new Modifier(-150)); 
            AddLog("弃暗投明 -150");
        }
#endregion

#region 速写美术家 TOY_916
        if(board.HasCardInHand(Card.Cards.TOY_916)
        ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.TOY_916, new Modifier(-99)); 
            AddLog("速写美术家 -99");
        }
#endregion

#region 加固护板 WW_334
        if(board.HasCardInHand(Card.Cards.WW_334)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WW_334, new Modifier(-99)); 
            AddLog("加固护板 -99");
        }
#endregion

#region 达拉然宣传单  WORK_050t
        if(board.HasCardInHand(Card.Cards.WORK_050t)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WORK_050t, new Modifier(-99)); 
            AddLog("达拉然宣传单 -99");
        }
#endregion

#region 安戈洛宣传单  WORK_050
        if(board.HasCardInHand(Card.Cards.WORK_050)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WORK_050, new Modifier(-99)); 
            AddLog("安戈洛宣传单 -99");
        }
#endregion

#region 攀上新高  VAC_949
        if(board.HasCardInHand(Card.Cards.VAC_949)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.VAC_949, new Modifier(-99)); 
            AddLog("攀上新高 -99");
        }
#endregion

#region 最后期限  WORK_007
        if(board.HasCardInHand(Card.Cards.WORK_007)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WORK_007, new Modifier(-99)); 
            AddLog("最后期限 -99");
        }
#endregion

#region 错位化石  DEEP_001
        if(board.HasCardInHand(Card.Cards.DEEP_001)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DEEP_001, new Modifier(-99)); 
            AddLog("错位化石 -99");
        }
#endregion

#region 泰坦锻造陷阱  TTN_302
        if(board.HasCardInHand(Card.Cards.TTN_302)
        ){
            p.ForgeModifiers.AddOrUpdate(Card.Cards.TTN_302, new Modifier(-350)); 
            AddLog("泰坦锻造陷阱锻造");
        }
#endregion
...
#region 遥控狂潮  TOY_354
        if(board.HasCardInHand(Card.Cards.TOY_354)
            ...
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TOY_354, new Modifier(-150)); 
            AddLog("遥控狂潮-150");
        }
#endregion

#region 冰冻陷阱  CORE_EX1_611
        if(board.HasCardInHand(Card.Cards.CORE_EX1_611)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_EX1_611, new Modifier(-150)); 
            AddLog("冰冻陷阱-150");
        }
#endregion

#region 泰坦锻造陷阱  TTN_302t
        if(board.HasCardInHand(Card.Cards.TTN_302t)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TTN_302t, new Modifier(-150)); 
            AddLog("泰坦锻造陷阱 -150");
        }
#endregion
...
#region 迷彩坐骑  WW_810
        if(board.HasCardInHand(Card.Cards.TTN_302t)
            ...
            foreach (var item in board.MinionFriend)
            {
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WW_810, new Modifier(999, item.Template.Id)); 
                 AddLog("迷彩坐骑 -150");
            }
        }
#endregion

#region 地幔塑型者  DEEP_004
        if(board.Hand.Exists(x=>x.CurrentCost<=4 && x.Template.Id==Card.Cards.DEEP_004)
        ){
           p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DEEP_004, new Modifier(-99)); 
            AddLog("迷彩坐骑 -150");
        }else{
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DEEP_004, new Modifier(350)); 
        }
#endregion

#region 小型法术翡翠  CORE_FB_Champs_LOOT_080 
        if(board.HasCardInHand(Card.Cards.CORE_FB_Champs_LOOT_080 )
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_FB_Champs_LOOT_080 , new Modifier(-150)); 
            AddLog("小型法术翡翠 -150");
        }
#endregion

#region 量产品9号  MIS_914
        if(board.HasCardInHand(Card.Cards.MIS_914)
        ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.MIS_914, new Modifier(-150)); 
            AddLog("量产品9号 -150");
        }
#endregion

#region 牵星短弓  TTN_088
        if(board.HasCardInHand(Card.Cards.TTN_088)
        ){
            p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.TTN_088, new Modifier(-150)); 
            AddLog("牵星短弓 -150");
        }
#endregion

#region 打卡  WORK_022
        if(board.HasCardInHand(Card.Cards.WORK_022)
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WORK_022, new Modifier(-150)); 
            AddLog("打卡 -150");
        }
#endregion

#region 爆破工头索格伦  WW_372
        if(board.HasCardInHand(Card.Cards.WW_372)
        ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WW_372, new Modifier(-150)); 
            AddLog("爆破工头索格伦 -150");
        }
#endregion

#region 遥控器  TOY_358
        if(board.HasCardInHand(Card.Cards.TOY_358)
        // 手上没武器
        &&board.WeaponFriend==null
        ){
            p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.TOY_358, new Modifier(-20)); 
            AddLog("遥控器 -20");
        }
#endregion

#region 迷彩坐骑  WW_810
        if(board.HasCardInHand(Card.Cards.WW_810)
        ){
            // 遍历友方随从
            foreach (var item in board.MinionFriend)
            {
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WW_810, new Modifier(-99, item.Template.Id)); 
                 AddLog("迷彩坐骑 -150");
            }
        }
#endregion

#region 蛮熊搬家  WORK_024
        if(board.HasCardInHand(Card.Cards.WORK_024)
         // 手牌数小于等于7
        &&HandCount<=7
        ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.WORK_024, new Modifier(-99));
            AddLog("蛮熊搬家 -99");
        }
#endregion
...
#region 质量保证   TOY_605
        ...
        &&HandCount<=7
            ...
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.TOY_605, new Modifier(-99));
            AddLog("质量保证 -99");
        }
#endregion
