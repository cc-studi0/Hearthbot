...
public class ArenaTrayDisplay : MonoBehaviour
{
	...
	public void ActivateKey()
	{
		...
		Action<object> value = delegate(object amount)
		{
			KeyGlowMat.SetFloat("_FxIntensity", (float)amount);
		};
		Hashtable tweenHashTable = iTweenManager.Get().GetTweenHashTable();
		tweenHashTable.Add("time", 2f);
		tweenHashTable.Add("from", 0f);
		tweenHashTable.Add("to", 1f);
		tweenHashTable.Add("easetype", iTween.EaseType.easeInOutBack);
		tweenHashTable.Add("onupdate", value);
		tweenHashTable.Add("onupdatetarget", m_TheKeySelectionGlow);
		iTween.ValueTo(m_TheKeySelectionGlow, tweenHashTable);
	...
	private IEnumerator AnimateKeyTransition(int rank)
	{
		...
		{
			oldKeySelectionGlowRenderer.GetMaterial().mainTexture = arenaKeyVisualData.m_SelectionGlowTexture;
		}
		...
		Hashtable tweenHashTable = iTweenManager.Get().GetTweenHashTable();
		tweenHashTable.Add("time", m_TheKeyTransitionFadeInTime);
		tweenHashTable.Add("from", 0f);
		tweenHashTable.Add("to", 1.5f);
		tweenHashTable.Add("easetype", iTween.EaseType.easeInCubic);
		tweenHashTable.Add("onupdate", value);
		tweenHashTable.Add("onupdatetarget", m_TheKeyOldSelectionGlow);
		iTween.ValueTo(m_TheKeyOldSelectionGlow, tweenHashTable);
		...
		Action<object> value2 = delegate(object amount)
		{
			KeyGlowMat.SetFloat("_FxIntensity", (float)amount);
		};
		Hashtable tweenHashTable2 = iTweenManager.Get().GetTweenHashTable();
		tweenHashTable2.Add("time", m_TheKeyTransitionFadeOutTime);
		tweenHashTable2.Add("from", 1.5f);
		tweenHashTable2.Add("to", 0f);
		tweenHashTable2.Add("easetype", iTween.EaseType.easeOutCubic);
		tweenHashTable2.Add("onupdate", value2);
		tweenHashTable2.Add("onupdatetarget", m_TheKeySelectionGlow);
		iTween.ValueTo(m_TheKeySelectionGlow, tweenHashTable2);
	...
	private void OnRewardBoxesDone()
	{
		if (!(this == null) && !(base.gameObject == null))
		{
			...
			{
				Network.Get().AckDraftRewards(draftManager.GetDraftDeck().ID, draftManager.GetSlot(), DraftManager.Get().IsUnderground());
			}
