...
public class EndGameTwoScoop : MonoBehaviour
{
	...
	protected virtual string GetActorName()
	{
		...
		if (entity != null)
		{
			string nameWithPremiumType = ActorNames.GetNameWithPremiumType(ActorNames.ACTOR_ASSET.PLAY_HERO, entity.GetPremiumType(), entity.GetCardId(), entity.GetController());
			if (nameWithPremiumType != null)
			{
	...
	private void OnTeammateHeroActorLoaded(AssetReference assetRef, GameObject go, object callbackData)
	{
		...
		Entity teammateHero = TeammateBoardViewer.Get().GetTeammateHero();
		if (teammateHero != null && teammateHero.GetCard() != null)
		{
			m_teammateHeroActor.SetPremium(teammateHero.GetCard().GetPremium());
		}
		m_teammateHeroActor.SetActorSpecificOverlayActive(teammateHero == null);
		m_teammateHeroActor.UpdateAllComponents();
		if (teammateHero == null)
		{
			m_teammateHeroActor.m_healthObject.SetActive(value: false);
