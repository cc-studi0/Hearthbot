...
public class EndGameScreen : MonoBehaviour
{
	...
	[Tooltip("Default transform override")]
	public TransformOverride m_transformOverride;

	[Tooltip("Used during the pet end game screen")]
	public TransformOverride m_petTransformOverride;
	...
	private bool m_isShowingPetRewards;

	private bool m_isShowingProgressionQuestRewards;
	...
	private SceneMgr.Mode? m_overrideNextMode;

	private string[] m_deepLinkModeOverride;

	private DeepLinkManager.DeepLinkSource m_deepLinkOverrideSource;

	protected virtual void Awake()
	{
		s_instance = this;
		if (m_transformOverride != null)
		{
			m_transformOverride.enabled = true;
		}
		if (m_petTransformOverride != null)
		{
			m_petTransformOverride.enabled = false;
		}
	...
	public virtual void Show()
	{
		...
		if (GameState.Get().HasPetEndgame())
		{
			if (m_petTransformOverride != null)
			{
				if (m_transformOverride != null)
				{
					m_transformOverride.enabled = false;
				}
				m_petTransformOverride.enabled = true;
			}
			else
			{
				Log.EndOfGame.PrintWarning("Attempted to use pet transform override but none was found");
			}
		}
		if (GameState.Get() != null && !GameState.Get().HasPetEndgame())
		{
			m_screenEffectsHandle.StartEffect(ScreenEffectParameters.BlurVignetteDesaturatePerspective);
		}
		else
		{
			m_screenEffectsHandle.StartEffect(ScreenEffectParameters.None);
		}
	...
	public void OverrideNextMode(SceneMgr.Mode overrideMode)
	{
		m_overrideNextMode = overrideMode;
	}

	public void DeepLinkOverrideNextMode(string[] deeplink, DeepLinkManager.DeepLinkSource source)
	{
		m_deepLinkModeOverride = deeplink;
		m_deepLinkOverrideSource = source;
	}
	...
	private void OnTutorialProgressScreenCallback(AssetReference assetRef, GameObject go, object callbackData)
	{
		...
		if (component != null)
		{
			component.SetEndGameScreen(this);
	...
	protected void BackToMode(SceneMgr.Mode mode)
	{
		...
		if (!m_hasAlreadySetMode)
		{
			...
			StartCoroutine((m_deepLinkModeOverride == null) ? ToMode(mode) : DeepLinkToMode(m_deepLinkModeOverride, m_deepLinkOverrideSource));
			Navigation.Clear();
		}
	}
	...
	private IEnumerator DeepLinkToMode(string[] deepLink, DeepLinkManager.DeepLinkSource source)
	{
		yield return new WaitForSeconds(0.5f);
		DeepLinkManager.ExecuteDeepLink(deepLink, source, 0);
	}

	private void ReturnToPreviousMode()
	{
		SceneMgr.Mode mode = SceneMgr.Mode.INVALID;
		if (m_deepLinkModeOverride == null)
		{
			mode = m_overrideNextMode ?? GameMgr.Get().GetPostGameSceneMode();
			m_overrideNextMode = null;
	...
	protected void HideTwoScoop()
	{
		if (m_twoScoop.IsShown())
		{
			...
			if (InputManager.Get() != null && GameMgr.Get() != null && GameMgr.Get().IsSpectator())
			{
	...
	protected void ShowJournalGameOnlyXpGains()
	{
		...
		rewardXpNotificationManager.SetTrackTypeFilter(RewardXpNotificationManager.JournalFilteredTrackTypes);
		if (rewardXpNotificationManager.HasXpGainsToShow())
		{
			m_shouldShowRewardXpGains = true;
			rewardXpNotificationManager.InitEndOfGameFlow(null);
	...
	{
		if (m_isShowingProgressionQuestRewards)
		{
			return true;
		}
		if (!QuestManager.Get().ShowNextReward(delegate
		{
			m_isShowingProgressionQuestRewards = false;
			ContinueEvents();
		}))
		{
			return false;
		}
		m_isShowingProgressionQuestRewards = true;
		return true;
	}
	...
	protected bool ShowNextCompletedQuest()
	{
		...
		using (new ShowNextCompletedQuestLock(this))
		{
			...
			if (QuestToast.IsReplacementRewardScrollActive())
			{
				QuestToast.GetCurrentReplacementRewardScroll().Hide();
			}
	...
	protected bool ShowJournalXpGains()
	{
		RewardXpNotificationManager rewardXpNotificationManager = RewardXpNotificationManager.Get();
		if (rewardXpNotificationManager == null)
		{
	...
	protected bool ShowPetXpGains()
	{
		...
		rewardXpNotificationManager.SetTrackTypeFilter(RewardXpNotificationManager.PetFilteredTrackTypes);
		if (rewardXpNotificationManager.IsShowingXpGains)
		{
			rewardXpNotificationManager.TerminateEarly();
			return false;
		}
	...
	protected bool ShowPetRewards()
	{
		if (m_isShowingPetRewards)
		{
			return true;
		}
		if (!PetsManager.Get().ShowNextReward(delegate
		{
			m_isShowingPetRewards = false;
			ContinueEvents();
		}))
		{
			return false;
		}
		m_isShowingPetRewards = true;
		return true;
	}
