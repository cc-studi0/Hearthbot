...
public class SceneMgr : IService, IHasUpdate
{
	...
	private void WillReset()
	{
		...
		if (m_mode != Mode.FATAL_ERROR)
		{
			...
			PostUnloadCleanup((scene != null) ? scene.SceneName : null);
		}
		QueueLoadBoxJob();
		Log.Reset.Print("\tSceneMgr.WillReset() completed");
	}

	public void SetNextMode(Mode mode, TransitionHandlerType transitionHandler = TransitionHandlerType.SCENEMGR, OnSceneLoadCompleteForSceneDrivenTransition onLoadCompleteCallback = null, object sceneTransitionPayload = null, bool reloadMode = false)
	{
		if (IsModeRequested(Mode.FATAL_ERROR))
		{
			return;
	...
	{
		Mode mode = GetMode();
		if (mode != Mode.LETTUCE_VILLAGE && mode != Mode.LETTUCE_BOUNTY_BOARD && mode != Mode.LETTUCE_MAP && mode != Mode.LETTUCE_PLAY && mode != Mode.LETTUCE_COLLECTION && mode != Mode.LETTUCE_COOP && mode != Mode.LETTUCE_FRIENDLY && mode != Mode.LETTUCE_BOUNTY_TEAM_SELECT)
		{
			return mode == Mode.LETTUCE_PACK_OPENING;
		}
		return true;
	}
	...
	private IEnumerator SwitchMode()
	{
		...
		{
			Log.All.PrintError("SwitchMode1 cannot complete because of null.");
		...
		try
		{
			...
			PostUnloadCleanup((m_previousScene != null) ? m_previousScene.SceneName : null);
		...
		{
			Log.All.PrintError("SwitchMode2 cannot complete because of null.");
			ExceptionReporter.Get().ReportCaughtException(exception2);
		}
	}
	...
	{
		IJobDependency[] dependencies = HearthstoneJobs.BuildDependencies(typeof(SceneMgr), typeof(IAssetLoader), typeof(NetCache), typeof(RewardTrackManager), new WaitForGameDownloadManagerAvailable(), new WaitForSplashScreen());
	...
	private void OnFatalError(FatalErrorMessage message, object userData)
	{
		...
		if (!ReconnectMgr.IsReconnectAllowed(message))
		{
			...
			else
			{
				...
				if (ServiceManager.TryGet<ReconnectMgr>(out var service))
				{
	...
	public bool DoesCurrentSceneSupportLuckyDrawStateUpdate()
	{
		switch (m_mode)
		{
		case Mode.STARTUP:
